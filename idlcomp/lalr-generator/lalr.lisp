...
;;; Roland Averkamp 18.3.2001
;;; the parser used to generate tail-recursive code, there was no problem when parsing small inputs,
;;; but with big input clisp ran out of stackspace, this is changed now.
;;; the error procedure gas as arguments now the symbols which can be shifted and reduced (????)
;;; I hope this will help if there is a pareser error
;;; I added 
...
(defvar      gepsilons)
(defvar      gexpansions)
(defvar      *lalr-debug* T "Inserts debugging code into parser if non-NIL")
(defvar      stateList '())
       ...
       (labels ((translateShift (shift)
         ...
         `(,(state-name state) ()
           (case (input-peek)
             ...
             (otherwise (funcall parse-error (quote ,(mapcar #'shift-cat (state-shifts state))) (quote ,(mapcar #'item-la reduces)))))))))
                ...
                (reduce-cat (name cat ndaughters action)
                            (if (eq cat ',*TOPCAT*)
                              ...
                              (let ((daughter-values '())
                                ...
                                state)))   ;;;; change, replace  (funcall state)))  ****
                ,@(mapcar #'translateState stateList))
         (do ((s (function ,(state-name (first stateList))) (funcall s)))
	     ((not (functionp s)) s)
	   )))))


; this is the stuff which used begin in the line marked with ****
;                       (funcall state))))
;                ,@(mapcar #'translateState stateList))
...
;;; added by Roland Averkamp 18.3.2001 
...
; just preprocessing the rules, I prefer another input format
(defun rolands-make-parser (rules tokens)
  (let ((transformed-rules (let (a) (dolist (r (mapcar #'flatten-rule rules))
				     (setf a (nconc a r)))
			       a)))
    (make-parser transformed-rules tokens nil)))
