...
(define-test-suite "Test IFR export"
  ...
  (define-test "servant-wrapper local/remote"
    (let* ((poa rootpoa)
      ...
      (let* ((data (list 12 "foo" interface
        ...
        (ensure-typep (elt rdata 2) 'corba:interfacedef-proxy)
        (ensure-typep (elt (elt rdata 3) 1) 'CORBA:Proxy))
      (let ((struct (corba:unionmember
                     :name "foo"
                     :label 123
                     :type CORBA:tc_string
                     :type_def (op:get_primitive repository :pk_string))))
        (let ((rs (make-remote wrapper struct)))
          (ensure-typep rs 'corba:unionmember)
          (ensure-typep (op:type_def rs) 'corba:primitivedef-proxy)
  ...
  (define-test "servant-wrapper invoke"
    (let* ((poa (handler-case 
      ...
      ;; Add some objects to the repository
      (op:create_interface
       (op:create_module repository "IDL:omg.org/CORBA:1.0" "CORBA" "1.0")
       "IDL:omg.org/CORBA/StringDef:1.0" "StringDef" "1.0" nil)

      (let ((repository-proxy (make-remote servant repository)))
        (ensure-pattern repository-proxy 
                        (repository-pattern 
                         "CORBA::StringDef" (def-pattern :dk_interface
      ...
      (let ((r (make-remote servant repository)))
        ...
        (let ((e (op:create_exception r "IDL:my/Exc:1.0" "Exc" "1.0" nil)))
          ...
          (let ((l (op:members e)))
            (ensure-typep l 'corba:structmemberseq)
        ...
        (let ((i (op:lookup_id r "IDL:foo:1.0")))
          (ensure-exception
           (op:create_operation i "IDL:my/Interface/op:1.0" "op" "1.0"
                                ...
                                :op_oneway
                                (list (corba:parameterdescription
